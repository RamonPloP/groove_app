     $(function() {
      get_data('/payment-types/list?page=0')
    });
...
function update_table(data) {
    ...
    $.each(data, function (i, element) {
        let payment_type = []
        payment_type.push(element.id)
        payment_type.push(element.name)
        payment_type.push('<button type="button" class="btn btn-outline-primary" onclick="show_info('+element.id +', this)">'+
                            '<i class="fas fa-edit"></i>'+
                        '</button>'+'<button type="button" class="btn btn-outline-danger" onclick="delete_payment_type('+element.id +', this)">'+
                            '<i class="fas fa-trash"></i>'+
                        '</button>')
        payment_types.push(payment_type)
    })
...
function delete_payment_type(id) {
    ...
    $('#confirmDeleteButton').off('click').on('click', function() {
        axios({
            ...
            data: {payment_type_id: payment_typeToDelete}
        }).then(function (response) {
            toastr.success(response.data, { timeOut: 9500 });
            get_data('/payment-types/list?page=0');
